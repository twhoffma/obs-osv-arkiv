...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('archive_item_materials', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('item', models.ForeignKey(orm['archive.item'], null=False)),
            ('materials', models.ForeignKey(orm['archive.materials'], null=False))
        ))
        db.create_unique('archive_item_materials', ['item_id', 'materials_id'])

        # Adding M2M table for field keywords on 'Item'
        db.create_table('archive_item_keywords', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('item', models.ForeignKey(orm['archive.item'], null=False)),
            ('keywords', models.ForeignKey(orm['archive.keywords'], null=False))
        ))
        db.create_unique('archive_item_keywords', ['item_id', 'keywords_id'])


    def backwards(self, orm):
        # Removing M2M table for field materials on 'Item'
        db.delete_table('archive_item_materials')

        # Removing M2M table for field keywords on 'Item'
        db.delete_table('archive_item_keywords')
